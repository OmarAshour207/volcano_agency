        ...
        $(document).ready(function () {
            $('#mainphoto').dropzone({
                ...
                init: function () {
                    ...
                    var mock = { name: '{{ $category->$title }}', size: 2};
                    this.emit('addedfile', mock);
                    this.emit('thumbnail', mock, '{{ $category->blog_image }}');
            ...
            <div class="card card-form__body card-body">
                <form method="post" action="{{ route('categories.update', $category->id) }}">
                    ...
                    <div class="form-group">
                        <label for="icon"> {{ trans('admin.blog') }} / {{ trans('admin.icon') }}</label>
                        <input id="icon" name="icon" type="text" class="form-control" placeholder="{{ __('admin.icon') }}" value="{{ $category->icon }}">
                    </div>

                    <div class="form-group">
                        <label for="ar_name"> {{ trans('admin.blog') }} / {{ trans('admin.ar_name') }}</label>
                        <input id="ar_name" name="ar_name" type="text" class="form-control" placeholder="{{ __('admin.ar_name') }}" value="{{ $category->ar_name }}">
                    </div>
                    <div class="form-group">
                        <label for="en_name"> {{ trans('admin.blog') }} / {{ trans('admin.en_name') }}</label>
                        <input id="en_name" name="en_name" type="text" class="form-control" placeholder="{{ __('admin.en_name') }}" value="{{ $category->en_name }}">
                    </div>
